	...
	describe("Object: settings", () => {
		...
		describe("Function: fetchRaw()", () => {
			it("is returning settings properly", async () => {
				...
				const data = await DBHandler.settings.fetchRaw(guildData._id);
				expect(data).toEqual(toCompare);
			});
			...
			it("is handling errors", async () => {
				...
				const data = await DBHandler.settings.fetchRaw("invalidId");
		...
		describe("Function: fetchAll()", () => {
			it("is correctly fetciching all server datas", async () => {
				...
				const dbData = await DBHandler.settings.fetchAll();
		...
		describe("Function: remove()", () => {
			...
			it("is handling errors", async () => {
				...
				const data = await DBHandler.settings.remove("invalidId");
		...
		describe("Function: removeGuild()", () => {
			it("is removing guilds successfully", async () => {
				...
				const guildIsRemoved = await DBHandler.settings.removeGuild(
					guildData._id
				);
